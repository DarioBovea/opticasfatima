...
export default function TarjetaBono({ mostrarEnlace = true, className = "" }: Props) {
  return (
    <div
      className={`rounded-xl bg-cover bg-center p-9 shadow-header ${className}`}
      style={{
        backgroundImage:
      ...
      data-aos="zoom-in"
    >
      {/* .slide : 45% base → 50% en ≤820px */}
      <div className="relative w-full text-primary md:w-[45%] max-[820px]:!w-1/2">
        <h3 className="text-6xl font-extrabold max-[820px]:text-[2.5em]">BONO</h3>
        ...
        {mostrarEnlace && (
          <Link
            href="/bono-regalo"
            title="Bono Regalo"
            className="inline-block rounded-lg bg-primary px-4 py-2.5 font-bold uppercase text-white shadow-btn transition hover:bg-light max-[820px]:text-sm"
          >
            Leer Más
          </Link>
        )}
